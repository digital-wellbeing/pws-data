...
cat("Missing timestamps:", sum(is.na(dq$timeStamp)), "\n")
dq <- dq %>% 
  drop_na(timeStamp)
...
We then create a separate table of demographics. At the same time, we find and exclude people who responded to the demographics questions more than once. Responding more than once shouldn't be possible, so it indicates dishonest responding / participation.

```{r}
ids_qualtrics <- dq %>% 
  filter(is.na(promptCategory)) %>% 
  distinct(entityId, age, gender) %>% 
  add_count(entityId, sort = TRUE, name = "Demographics_responses")

ids_qualtrics %>% 
  distinct(entityId, age, gender, Demographics_responses) %>% 
  count(Demographics_responses, name = "N people") %>% 
  kable(caption = "Numbers of people responding to demographics X times")

ids_qualtrics <- ids_qualtrics %>% 
  filter(Demographics_responses == 1) %>% 
  select(entityId, age, gender)
...
events <- events %>% 
  bind_rows(
    tibble(
      ...
      Variable = "OxfordStudyLocalTimeStamp"
    )
  ) %>% 
  arrange(EventName)
...
join_summary %>% 
  ...
  geom_point(size = 1, alpha = .75, shape = 1) +
