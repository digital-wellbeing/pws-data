...
d <- d %>%
  ...
  mutate(
    time_as_int = sql('cast(extract(epoch from "Timestamp") as integer)'),
    pid_as_number = as.numeric(pid)
  ) %>% 
  window_order(pid_as_number, time_as_int) %>%
  fill(offset, .direction = "down") %>% 
  fill(offset, .direction = "up") %>% 
  ungroup() %>% 
  select(-pid_as_number, -time_as_int)
