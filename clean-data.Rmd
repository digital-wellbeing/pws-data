...
```{r qualtrics-example-duplicates}
dq %>% 
...
```{r playfab-ids}
# Identify test accounts (participated before launch date)
test_accounts <- tbl(con, "pws") %>% 
  filter(EventName == "player_logged_in") %>% 
  distinct(EntityId, Timestamp) %>% 
  collect() %>% 
  arrange(Timestamp) %>% 
  filter(Timestamp < as_date("2022-08-18")) %>% 
  pull(EntityId) %>% 
  unique()

# Get distinct entity IDs from playfab data, remove test accounts
ids_playfab <- tbl(con, "pws") %>% 
  ...
  filter(!(EntityId %in% test_accounts))
...
ids_qualtrics <- ids_qualtrics %>% 
  add_count(country) %>% 
  mutate(country = if_else(n < 10, NA_character_, country)) %>% 
  select(-n)
```
...
Take out invalid participants and non-relevant events, and rename event where responses were skipped to the same name where they were responded.
...
d <- compute(d, name = "pws_2")

# Count all events at this point
playfab_orig_count <- d %>% 
  count(EventName) %>% 
  collect() %>% 
  arrange(EventName)
```
...
Take out duplicated events by keeping only distinct event ids. Save results in a temporary table

```{r playfab-duplicates-delete}
counts_all <- d %>% 
  count(EventName) %>% 
  collect()

d <- d %>% 
  distinct(EventName, pid, EventId, .keep_all = TRUE) %>% 
  compute(name = "pws_distinct_events")

counts_distinct_events <- d %>% 
  count(EventName) %>% 
  collect()

left_join(
  counts_all,
  counts_distinct_events,
  by = "EventName"
) %>% 
  mutate(n_duplicates_removed = n.x - n.y) %>% 
  adorn_totals() %>% 
  mutate(across(starts_with("n"), ~number(as.numeric(.), big.mark = ","))) %>% 
  kable()
```
...
We work only on the survey event telemetry (because it is the only data to join)
...
```{r save-joined-duckdb}
# Overwrite survey tables in duckdb
playfab_survey_data_joined %>% 
...
left_join(playfab_orig_count, events_final_count) %>% 
  mutate(
    n_lost = n - rows,
    p_lost = percent(n_lost / n, .1)
  )
```
...
We also save them as compressed comma-separated tables

```{r write-all-csvs}
c(unique(events$EventName), "demographics") %>%
  .[-c("study_prompt_skipped", "clear_data_requested")] %>% 
  walk(
    ~tbl(duck, .x) %>%
      collect() %>%
